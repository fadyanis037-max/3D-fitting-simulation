 ...
 */
function place(landmark: Landmark, aspect: number, target: Vector3): void {
...
export class JointExtractor {
  ...
  update(landmarks: Landmark[], aspect: number): BodyJoints | null {
    ...
    place(landmarks[POSE.leftHip], aspect, joints.leftHip);
    place(landmarks[POSE.rightHip], aspect, joints.rightHip);
    place(landmarks[POSE.leftShoulder], aspect, joints.leftShoulder);
    place(landmarks[POSE.rightShoulder], aspect, joints.rightShoulder);
    ...
    if (joints.leftElbowVisible) place(landmarks[POSE.leftElbow], aspect, joints.leftElbow);
    if (joints.rightElbowVisible) place(landmarks[POSE.rightElbow], aspect, joints.rightElbow);
    if (joints.leftWristVisible) place(landmarks[POSE.leftWrist], aspect, joints.leftWrist);
    if (joints.rightWristVisible) place(landmarks[POSE.rightWrist], aspect, joints.rightWrist);
    if (joints.noseVisible) place(landmarks[POSE.nose], aspect, joints.nose);
